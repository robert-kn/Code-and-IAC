 ...
 * interface.
 * 
 * Once hints is set up properly, we declare a pointer to a struct addrinfo structure, which holds the return information from getaddrinfo(). We then call 
 * the getaddrinfo() function. The getaddrinfo() function has many uses, but for our purpose, it generates an address
 * that's suitable for bind(). To make it generate this, we must pass in the first parameter as NULL and have the 
