...
{     
    ...
    if (p) {
        protocol = url;
        *p = 0; /* store \0 in the postion that character : occupies*/
        p += 3; /* advance to first character of the hostname */
    ...
    *hostname = p;
    /* walk through to the end of the hostname */
    while (*p && *p != ':' && *p != '/' && *p != '#') 
        ++p;

    *port = "80";
    if (*p == ':') { /* if a colon exits, set the port to the value after the colon */
        *p++ = 0;
        *port = p;
    }
    while (*p && *p != '/' && *p != '#') 
